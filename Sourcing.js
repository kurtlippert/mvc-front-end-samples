...
RMS.UI.Administration.Sourcing = (function ($, sourcing) {
    ...
    sourcing.InitializeWizard = function (modelId) {
        ...
        function initializeSteps () {
            ...
            form.steps({
                ...
                onFinished: function (event, currentIndex) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: (modelId !== null) ? RP.Settings.SourcingUpdateConfiguration + "/" + modelId + "?model='" + JSON.stringify(model) + "'" : RP.Settings.SourcingAddConfiguration,
                        cache: false,
                        dataType: 'json',
                        data: passedData,
                        success: function () {
        ...
        function initializeGlobalCriteria () {
            ...
            var $operatorArea = form.find("div#operator-area").hide();
            ...
            var $operator = $operatorArea.find("input#operator");
            ...
            form.find("#btn-static-add").click(function () {
                ...
                if (wizardKendoValidator.validate()) {
                    ...
                    var operatorValue;
                    if ($operatorArea.is(":visible")) {
                        operatorValue = $operator.data("kendoDropDownList").text();
                    }
                    else {
                    ...
                    if ($startDateArea.is(":visible") && $endDateArea.is(":visible") && operatorValue === "Between") {
                        staticCriteriaDateStart = $startDate.val();
                        staticCriteriaDateEnd = $endDate.val();
                    }
                    else if ($startDateArea.is(":visible") && operatorValue !== "Between") {
                        staticCriteriaDateStart = $startDate.val();
                    }
    ...
    sourcing.GlobalCandidateFields = new function () {
        ...
        this.Change = function (e) {
            ...
            var $staticCriteriaOperatorArea = $sourcingWizard.find("div#operator-area").hide();
            var $startDateArea = $sourcingWizard.find("div#start-date-area").hide();
            var $endDateArea = $sourcingWizard.find("div#end-date-area").hide();

            // Need to know when to use a combobox / textbox vs. data-picker
            if (searchDataTypeId !== 1) {
                ...
                $staticCriteriaOperatorArea.show();

                // Want to reset the date areas
                $staticCriteriaOperatorArea.find("#operator").data("kendoDropDownList").select(0);
                $endDateArea.hide();
                $startDateArea.find("#start-date").val("");
                $endDateArea.find("#end-date").val("");

                sourcing.DisplayDateRange($staticCriteriaOperatorArea.find("#operator").val() === "between");
            }
        }
